...
public class BankSession extends Thread {
	...
	public boolean operate() {
		try {
			...
			case Protocol.REQUEST_GET_CARD:
				processGetCard(reader, writer);
				break;
			case Protocol.REQUEST_GET_ALL_CARDS:
				processGetAllCards(reader, writer);
				break;
	...
	private void processGetAllCards(BankReader reader, BankWriter writer) {
		Collection<Card> cards = model.getAll();
		writer.createList(cards);
	}

	private void processGetCard(BankReader reader, BankWriter writer) {
		long cardID = reader.getNumber();
		Card card = model.get(cardID);
